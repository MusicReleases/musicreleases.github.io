...
public class DatabaseController(IIndexedDbFactory dbFactory) : IDatabaseController
{
	...
	public async Task<SpotifyUserList<SpotifyArtist, SpotifyUserListUpdateArtists>?> GetArtists()
	{
		...
		{
			return GetArtistsDb(db);
		}
	}

	private SpotifyUserList<SpotifyArtist, SpotifyUserListUpdateArtists>? GetArtistsDb(SpotifyReleasesDb db)
	{
		...
		/*if (db.Update?.Artists is null)
		{
			return null;
		}

		var update = new SpotifyUserListUpdateArtists(db.Update.Artists.Value)
		{
			LastUpdateAlbums = db.Update.ReleasesAlbums,
			LastUpdateTracks = db.Update.ReleasesTracks,
			LastUpdateAppears = db.Update.ReleasesAppears,
			LastUpdateCompilations = db.Update.ReleasesCompilations,
			LastUpdatePodcasts = db.Update.ReleasesPodcasts,
		};*/
		var update = new SpotifyUserListUpdateArtists(DateTime.Now)
		{
			LastUpdateAlbums = DateTime.Now,
		};
	...
	public async Task SaveArtists(SpotifyUserList<SpotifyArtist, SpotifyUserListUpdateArtists> artists)
	{
		...
		using (var db = await _dbFactory.Create<SpotifyReleasesDb>())
		{
			/*db.Update ??= new SpotifyLastUpdateEntity(Guid.NewGuid());

			// update - update times
			db.Update.Artists = artists.Update!.LastUpdateMain;

			db.Update.ReleasesAlbums = artists.Update.LastUpdateAlbums;
			db.Update.ReleasesAppears = artists.Update.LastUpdateAppears;
			db.Update.ReleasesCompilations = artists.Update.LastUpdateCompilations;
			db.Update.ReleasesPodcasts = artists.Update.LastUpdatePodcasts;
			db.Update.ReleasesTracks = artists.Update.LastUpdateTracks;*/

...
public class DatabaseSpotifyArtistReleaseController(IDatabaseSpotifyController dbController, IDatabaseSpotifyReleaseController dbReleaseController, IDatabaseSpotifyUpdateController dbUpdateController, IDatabaseSpotifyArtistController dbArtistController) : IDatabaseSpotifyArtistReleaseController
{
	private readonly IndexedDbStore _dbTable = dbController.GetTable(DbStorageTablesSpotify.SpotifyArtistRelease);

	private readonly IDatabaseSpotifyReleaseController _dbReleaseController = dbReleaseController;
	...
	public async Task<SpotifyUserList<SpotifyRelease, SpotifyUserListUpdateRelease>?> Get(ISet<SpotifyArtist> artists, string userId)
	{
		...
		var releases = await _dbReleaseController.Get(releaseIds);
		if (releases is null)
		{
			return null;
		}
	...
	private async Task<ISet<SpotifyReleaseArtistsDbObject>> GetReleaseIds(ISet<SpotifyArtist> followedArtists)
	{
		...
		var allArtists = await _dbArtistController.GetAll();
	...
	private async Task SaveDb(ISet<SpotifyRelease> releases, string userId)
	{
		...
		// save new releases
		await _dbReleaseController.Save(newReleases);
		...
		await _dbArtistController.Save(newArtists);
	...
	private async Task Delete(ISet<SpotifyArtistReleaseEntity> artistReleasesDb)
	{
		foreach (var artistReleaseDb in artistReleasesDb)
		{
			...
			await _dbReleaseController.Delete(artistReleaseDb.ReleaseId);
		}
	}
}

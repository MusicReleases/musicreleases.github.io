...
internal abstract class SpotifyUserLinkEntityService<TUserIdEntity, TPayload>
	: SpotifyLinkOneKeyEntityService<TUserIdEntity, TPayload>, ISpotifyUserLinkEntityService<TPayload> where TUserIdEntity : class, ISpotifyDb, ISpotifyUserIdLinkEntity
	where TPayload : ISpotifyPayload
{
	public async Task<IReadOnlyCollection<TPayload>> GetByUserId(
		string userId,
	...
	public async Task SaveByUserId(
		string userId,
	...
	public async Task Save(
		string userId,
		IReadOnlyCollection<TPayload> items,
		CancellationToken ct)
	{

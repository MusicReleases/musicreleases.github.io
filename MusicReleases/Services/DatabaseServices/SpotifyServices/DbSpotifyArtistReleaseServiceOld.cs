...
public class DbSpotifyArtistReleaseServiceOld(IDbSpotifyServiceOld dbService, IDbSpotifyReleaseServiceOld dbReleaseService, IDbSpotifyUpdateServiceOld dbUpdateService, IDbSpotifyArtistService dbArtistService) : IDbSpotifyArtistReleaseServiceOld
{
	private readonly IndexedDbStore _dbTable = dbService.GetTable(DbStorageTablesSpotify.SpotifyArtistRelease);

	private readonly IDbSpotifyReleaseServiceOld _dbReleaseService = dbReleaseService;
	...
	public async Task<SpotifyUserList<SpotifyReleaseOld, SpotifyUserListUpdateRelease>?> Get(ISet<SpotifyArtist> artists, string userId)
	{
		// update
		...
		var releasesUpdate = new SpotifyUserList<SpotifyReleaseOld, SpotifyUserListUpdateRelease>(releases, update);
	...
	private async Task<ISet<SpotifyArtistReleaseEntityOld>> GetAllDb()
	{
		Console.WriteLine("db: get artist releases - start");
		var artistReleasesDb = _dbTable.GetAllAsync<SpotifyArtistReleaseEntityOld>();

		var artistReleases = new HashSet<SpotifyArtistReleaseEntityOld>();
	...
	public async Task Save(string userId, SpotifyUserList<SpotifyReleaseOld, SpotifyUserListUpdateRelease> releases)
	{
	...
	private async Task SaveDb(ISet<SpotifyReleaseOld> releases, string userId)
	{
		...
		var artistReleaseEntities = new HashSet<SpotifyArtistReleaseEntityOld>();
		foreach (var release in newReleases)
		{
			foreach (var artist in release.Artists)
			{
				var artistReleaseEntity = new SpotifyArtistReleaseEntityOld(artist.Id, release.Id);
	...
	private async Task Delete(ISet<SpotifyArtistReleaseEntityOld> artistReleasesDb)
	{
